          ...
          {[...Array(8)].map((_, col) => (
            <div key={col}>
              <input
                ...
                className="rounded-full appearance-none border border-white border-5 w-6 h-6 checked:bg-white checked:border-transparent transition-all duration-100 ease-in-out"
    ...
    <div className="flex flex-col space-y-5">
      <div className="flex flex-row justify-end">
        <div className="-mr-2.5">
          {columns.map((col) => (
            <span
              key={col}
              className="relative text-center text-gray-900 dark:text-white bg-blue-900 rounded-full py-1 px-3 space-x-tip-col"
            >
              {col}
            </span>
          ))}
      ...
      <div className="flex flex-row justify-center items-center space-x-4">
        <div className="space-y-1.5">
          {rows.map((row) => (
            <div key={row}>
              <p className="text-center text-gray-600 dark:text-white bg-blue-900 w-fit rounded-full py-1 px-3">
                {row}
              </p>
            </div>
          ))}
